...
class AddWaterNodeTopLevel(CTkToplevel):
    def __init__(self, master, on_connect=None):
        super().__init__(master)
        self.on_connect = on_connect

        # --- WINDOW CONFIG ---
        self.title("Add Water Quality Node")
        ...
        self.colors = {
            ...
            "water_accent": "#4CAFE8"  # aqua blue for water theme
        }
        ...
        CTkLabel(
            panel,
            text="TDS & temperature water quality monitoring",
            font=("Tahoma", 12),
        ...
        self.ip_entry = CTkEntry(
            panel,
            placeholder_text="Enter Water Node IPT ID",
        ...
        CTkLabel(
            info_box,
            text="ℹ️  Measures Total Dissolved Solids (TDS) for water purity\nand temperature using DS18B20 sensor.",
            font=("Tahoma", 11),
            text_color=self.colors["muted"],
            justify="left",
        ...
        self.connect_btn = CTkButton(
            panel,
            text="CONNECT WATER NODE",
            fg_color=self.colors["water_accent"],
            hover_color="#3a9ad0",
            text_color="white",
            height=46,
            corner_radius=12,
    ...
    def _connect(self):
        ...
        if not iptid or not pwd:
            messagebox.showwarning("Missing Data", "Please enter both Water Node ID and password.")
            self.connect_btn.configure(state='normal')
            return

        # Verify water node credentials
        real_check = iPT_UA_database.check_water_node_add_to_app(iptid, pwd)
        if real_check:
            iPT_UA_database.add_water_node_offline(iptid)
            ...
            try:
                ...
                if user_email:
                    ...
                    conn.execute(f"""
                        ...
                        WHERE water_node_name='{iptid}'
                    """)
                    connection.commit()
                    conn.close()
                    connection.close()
            except Exception as e:
                print(f"Error updating water node ownership: {e}")
            ...
            messagebox.showinfo(
                'SUCCESS',
                '💧 Water Quality Node Connected!\n\n'
                'Your water monitoring system is now active.\n'
                'Tracking: TDS (water purity) & temperature.'
            )
        else:
            messagebox.showwarning(
                'Authentication Failed',
                'Water Node ID or Password is incorrect.\n\n'
    ...
    def _forgot(self):
        messagebox.showinfo(
            "Forgot Password",
            "To reset your Water Node credentials:\n\n"
