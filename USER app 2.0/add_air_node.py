...
class AddAirNodeTopLevel(CTkToplevel):
    def __init__(self, master, on_connect=None):
        super().__init__(master)
        self.on_connect = on_connect

        # --- WINDOW CONFIG ---
        self.title("Add Air Quality Node")
        ...
        self.colors = {
            ...
            "air_accent": "#5BA3D0"  # light blue for air quality theme
        }
        ...
        CTkLabel(
            panel,
            text="Multi-sensor air quality monitoring system",
            font=("Tahoma", 12),
        ...
        self.ip_entry = CTkEntry(
            panel,
            placeholder_text="Enter Air Node IPT ID",
        ...
        CTkLabel(
            info_box,
            text="ℹ️  Monitors CO, smoke, gas levels (MQ2/4/6/7/135),\ntemperature, and humidity with DHT22 sensor.",
            font=("Tahoma", 11),
            text_color=self.colors["muted"],
            justify="left",
        ...
        self.connect_btn = CTkButton(
            panel,
            text="CONNECT AIR NODE",
            fg_color=self.colors["air_accent"],
            hover_color="#4a8bb8",
            text_color="white",
            height=46,
            corner_radius=12,
    ...
    def _connect(self):
        ...
        if not iptid or not pwd:
            messagebox.showwarning("Missing Data", "Please enter both Air Node ID and password.")
            self.connect_btn.configure(state='normal')
            return

        # Verify air node credentials
        real_check = iPT_UA_database.check_air_node_add_to_app(iptid, pwd)
        if real_check:
            iPT_UA_database.add_air_node_offline(iptid)
            ...
            try:
                ...
                if user_email:
                    ...
                    conn.execute(f"""
                        ...
                        WHERE air_node_name='{iptid}'
                    """)
                    connection.commit()
                    conn.close()
                    connection.close()
            except Exception as e:
                print(f"Error updating air node ownership: {e}")
            ...
            messagebox.showinfo(
                'SUCCESS',
                '💨 Air Quality Node Connected!\n\n'
                'Your multi-sensor air monitoring system is now active.\n'
                'Tracking: CO, smoke, gases, temperature & humidity.'
            )
        else:
            messagebox.showwarning(
                'Authentication Failed',
                'Air Node ID or Password is incorrect.\n\n'
    ...
    def _forgot(self):
        messagebox.showinfo(
            "Forgot Password",
            "To reset your Air Node credentials:\n\n"
